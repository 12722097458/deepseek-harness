...
PowerShell 启动器将 `deploy/` 的父目录解析为仓库根目录，并使用带 BOM 的 UTF-8 编码，确保 Windows PowerShell 5.1 能正确解析中文提示。

启动前，Windows 和 macOS/Linux 启动器会终止占用所选端口的所有进程，避免旧服务导致重复启动失败。该行为可能终止占用该端口的其他本地服务，调用者需要确认端口属于本项目。
